...
def time_series_real_eval(X, y, threshold=0.5, starting_capital=1000, nb_contracts=1, results={}):
    ...
    def lgb_objective(trial):
        ...
        dtrain = lgb.Dataset(X_train.drop(columns=['strike', 'datetime', 'expire_date']), label=y_train['target'], categorical_feature=CATEGORICAL_COLS)
        dtune = lgb.Dataset(X_tune.drop(columns=['strike', 'datetime', 'expire_date']), label=y_tune['target'], categorical_feature=CATEGORICAL_COLS)
        ...
        y_val_pred_proba = model.predict(X_tune.drop(columns=['strike', 'datetime', 'expire_date']))
        ...
        y_train_pred_proba = model.predict(X_train.drop(columns=['strike', 'datetime', 'expire_date']))
    ...
    lgb_train = lgb.Dataset(X_train.drop(columns=['strike', 'datetime', 'expire_date']), label=y_train['target'], categorical_feature=CATEGORICAL_COLS)
    model = lgb.train(
        best_params,
        lgb_train,
        num_boost_round=1000, # it takes less time to reach min l2
        valid_sets=[lgb_train, lgb_train],
        callbacks=[lgb.early_stopping(stopping_rounds=100), lgb.log_evaluation(100)],
    )

    # 📈 Final test evaluation with anomaly filtering
    X_test_features = X_test.drop(columns=['strike', 'datetime', 'expire_date']).copy()
    y_pred_proba = model.predict(X_test_features)

    invalid_mask = (X_test['time_to_expiry'] >= 1500) | (X_test['moneyness'] >= 1.11)
    y_pred_proba[invalid_mask.values] = 0.0

    # Re calculation of best thr with X_tune data
    y_val_pred_proba = model.predict(X_tune.drop(columns=['strike', 'datetime', 'expire_date']))
    invalid_mask_tune = (X_tune['time_to_expiry'] >= 1500) | (X_tune['moneyness'] >= 1.11)
    y_val_pred_proba[invalid_mask_tune.values] = 0.0
    ...
    y_pred = (y_pred_proba >= best_val_threshold).astype(int)


    train_proba = model.predict(X_train.drop(columns=['strike', 'datetime', 'expire_date']))
    X_train["pred_proba"] = train_proba
    positive_high_conf = X_train[(y_train['target'] == 1) & (X_train['pred_proba'] >= best_val_threshold)]
    ...
    train_proba = model.predict(X_train.drop(columns=['strike', 'datetime', 'expire_date'], axis=1).copy())
    best_train_prediction = (train_proba >= best_val_threshold).astype(int)
    best_train_precision = precision_score(y_train['target'], best_train_prediction, pos_label=1, zero_division=0.0)
    best_test_precision = precision_score(y_test["target"], y_pred, pos_label=1, zero_division=0.0)

    print(f"best_threshold: {best_val_threshold}")
    print(f"best_val_precision: {best_val_precision}")
    print(f"best_test_precision: {best_test_precision}")
    ...
    return final_capital, cap_history, trades, results, (model, best_val_threshold, best_test_precision, y_pred_proba)
